...
import { contactSection } from "@/config/websites-en/content";
...
function validate(values: FormState): Errors {
  const errors: Errors = {};
  if (!values.nome.trim()) errors.nome = "Please enter your name.";
  if (!values.email.trim()) {
    errors.email = "Please enter your email.";
  } else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(values.email.trim())) {
    errors.email = "Please enter a valid email address.";
  }
  if (!values.mensagem.trim()) errors.mensagem = "Please write a short message.";
  if (!values.consentimento)
    errors.consentimento = "Please authorise contact to continue.";
  return errors;
}
...
export function ContactForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setStatus("success");
      setServerMessage(
        "Your enquiry has been sent. We will reply by email."
      );
    } catch (error) {
      setStatus("error");
      setServerMessage(
        error instanceof Error
          ? error.message
          : "The enquiry could not be sent. Tente novamente."
      );
    }
  }
        ...
        <div className="mx-auto grid max-w-5xl gap-12 lg:grid-cols-[1fr_1.3fr]">
          <FadeIn className="self-start rounded-[1.7rem] border-[3px] border-night bg-[#f6dfb6] p-6 shadow-[7px_7px_0_#C69A44] sm:p-8">
            ...
            <ul className="mt-8 space-y-3 text-sm text-slate">
              {[
                "Initial review with no commitment",
                "Clear opportunities for improvement",
                "Free initial mockup para negócios selecionados",
              ].map((item) => (
              ...
              <div className="grid gap-5 sm:grid-cols-2">
                <div>
                  <label htmlFor="nome" className="mb-1.5 block text-sm font-medium text-ink">
                    Name <span className="text-emerald-dark">*</span>
                  </label>
                  <input
                    id="nome"
                    name="nome"
                ...
                <div className="sm:col-span-2">
                  <label htmlFor="mensagem" className="mb-1.5 block text-sm font-medium text-ink">
                    Message <span className="text-emerald-dark">*</span>
                  </label>
                  <textarea
                    id="mensagem"
                    name="mensagem"
